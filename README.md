...
This aggregator subdivides the space by binning in one continuous variable, `X`, and then further subdivides it by binning in `Y` Then it counts. The result is a two-dimensional histogram, like both of the following:
...
| [Bag](http://diana-hep.org/histogrammar/scala/0.1/index.html#org.dianahep.histogrammar.Bag$)             | Accumulate raw data up to an optional limit, at which point only the total number are preserved. |
