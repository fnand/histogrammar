...
class PurePython(object):
    ...
    def test(self, fillvalue, assertion):
        for assertion in self.autoassertions:
...
if __name__ == "__main__":
    ...
    purePython[-1].test(simple[0], "self.assertEqual({}.toJson(), {})".format(PurePython.var, {"type": "Count", "data": 1.0}))
    purePython[-1].test(simple[1], "self.assertEqual({}.toJson(), {})".format(PurePython.var, {"type": "Count", "data": 2.0}))
    purePython[-1].test(simple[2], "self.assertEqual({}.toJson(), {})".format(PurePython.var, {"type": "Count", "data": 3.0}))

    template = open("TEMPLATES/PurePython.py").read()
    open("../python/test/test.py", "w").write(template.replace("{{TESTS}}", "\n\n".join(p.string() for p in purePython)))
